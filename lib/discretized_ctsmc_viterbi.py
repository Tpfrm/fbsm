...
class Discretized_CTSMC_Viterbi:
    ...
    def kolmogorov_setup(self, tau_max, dt, step_callback=None, phis=None):
        # must run setup before we can forward the marginal
        N = int(0.1*tau_max/dt)
        #print('N = ' + str(0.005*tau_max/dt) + ', scaled_tau_max = ' + str(0.005*tau_max) + ', dt = ' + str(dt))
        if (self.k is None):
            self.k = _Kolmogorov()
            ...
            self.k.dt = dt
            self.k.tau_max = 0.1*tau_max
            self.k.n_steps = N
            self.k.ts = np.linspace(0, 0.1*tau_max, self.k.n_steps)
            ts = self.k.ts
            self.k.phis = phis
            self.k.phi_counter = 0
            self.k.new_phi = False
            if self.k.phis is None:
                self.k.new_phi = True
                self.k.phis = np.zeros([self.num_states, int(tau_max/dt)])
            for s in range(self.num_states):
                if self.k.new_phi:
                    ...
                    self.k.p_t[s][0] = self.k.phis[s][0]
                self.k.next_phis = np.zeros([self.num_states, int(tau_max/dt)])
    ...
    def __advance_step(self, p_t, l, dt):
        ...
        r_t = l*p_t
        for s in range(self.num_states):
            q_t[s] = Discretized_CTSMC_Viterbi.___shift(p_t[s] - r_t[s]*dt, 1)
            for ss in range(self.num_states):
                if (ss != s):
                    #q_t[s][0] += self.jp_dists[ss][s]*np.sum(r_t[ss])*dt
                    self.k.next_phis[s, self.k.phi_counter] += self.jp_dists[ss][s]*np.sum(r_t[ss])*dt
            q_t[s][0] = self.k.phis[s, self.k.phi_counter]
        self.k.phi_counter += 1
